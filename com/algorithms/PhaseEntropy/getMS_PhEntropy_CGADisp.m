...
function [PhEn] = getMS_PhEntropy_CGADisp(data,noSector,scale)

%%Parameters
%% data is N by 1 matrix
%% noSector is an integer

%%truncate the last portion of data that dont fit on scale

%extraDataP = mod(size(data,1),scale);
%data = data(1:end-extraDataP,1);
...
%% step 2_extra...    
%%% coarse grain in angular displacement....
%%% sort the angular displacement...

theta=sort(theta);

%thetaCG=nan(size(theta,1)/scale,1);
thetaCG=nan(floor(size(theta,1)/scale),1);
%chkList=[];
%for j=1:size(thetaCG,1)/scale
for j=1:floor(size(thetaCG,1)/scale)
    endIndex=j*scale;
    startIndex=(j-1)*scale+1;
    thetaCG(j,1)=nanmean(theta(startIndex:endIndex,1));
    %chkList=[chkList;startIndex endIndex thetaCG(j,1)];
end
...
for i=1:k
        ...
        S_theta(i,1)=sum(thetaCG(thetaCG(:,1)>=startAngle & thetaCG(:,1)<startAngle + angleSpan,1));
