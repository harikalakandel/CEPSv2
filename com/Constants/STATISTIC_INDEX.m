classdef STATISTIC_INDEX
    ...
    methods(Static)
        ...
        %% list of all measures that have timeseries output
        %function out = getMultiScaleList(obj)
        function out = getTimeSeriesList()
            out = [STATISTIC_INDEX.FFT,STATISTIC_INDEX.FastLomb];
        end
        %function out = isMultiScale(obj,index)
        function out = isTimeSeriesMeasure(index)
            out = sum(STATISTIC_INDEX.getTimeSeriesList()==index)>0;
        end
        
        function out = getNotImplementedList()
            out=[STATISTIC_INDEX.AverageFalseNeighbours_AFN STATISTIC_INDEX.DFA  STATISTIC_INDEX.DifferentialEntropy_DiffEn];
        end
        
        function out = isImplemented(index)
            out = sum(STATISTIC_INDEX.getNotImplementedList()==index)==0;
        end
