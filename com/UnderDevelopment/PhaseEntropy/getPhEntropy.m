...
function [PhEn] = getPhEntropy(data,noSector)
...
S_theta = nan(k,1);
%% adjust number of theta
Nnew = N-2;

%% number of points startIndex EndIndex
M=nan(k,3);
%% 3.1 number of points in each sector
M(1:end-1,1)=ceil(Nnew/k);
%% the last sector will have only left over data points
M(end,1)= Nnew - sum(M(1:end-1,1));

%% 3.2 start Index for each sector
M(1,2)=1;
for i=2:k
    M(i,2)=sum(M(1:i-1,1))+1;
end
%% 3.3 end Index for each sector
for i=1:k-1
    M(i,3)=M(i+1,2)-1;
end
%% maximum length that can be considered Nnew
disp('[nPoints startI endStart]')
M(end,3)=Nnew;
disp(M)

%% calculation of S_theta
j=1;
for i=1:k
    startIndex = M(i,2);
    endIndex = M(i,3);
    ...
    %for j=startIndex:endIndex
        S_theta(i,1)=sum(theta(startIndex:endIndex));
        %index=index+1;
    %end
