function [SE,unique] = ShannonEn_Giuseppe(series,L,num_int)
%{
...
try
    ...
    cd('../matlabToolBox/Communications')
    [~,quants] = quantiz(series, partition, codebook);
    cd('../../Entropy_measures');
end
...
% I gave a look at the file
% “ShannonEn.m”
% I did not analyzed in details how the “p_i” are computed in this function. But if you trust this computation, then in order to compute the extropy it is enough to replace  
%  the last code 
% "SE = (-1) * ((p_i)*(log(p_i)).’);”
% by the following code
% "SExtropy = (-1) * ((1-p_i)*(log(1-p_i)).’);

SE = (-1) * ((1-p_i)*(log(1-p_i)).')
