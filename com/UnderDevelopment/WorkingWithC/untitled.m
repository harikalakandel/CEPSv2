...
function [renyi_mean] = getRenyEntropy(vecData,varargin)
% {
...
if isempty(probability)| pattern ~= last_pattern | tolerance ~= last_tolerance 
    ...
    probability = RenyiProbs(maxwords, pattern, tolerance);
end
...
renyi_stdev = (sum_xx / (dataReps - 1) - renyi_mean * renyi_mean)^(0.5);
