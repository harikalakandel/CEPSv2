...
Chk3 = {'uniquant','finegrain','modified','ampaware','weighted','edge'};
addRequired(p,'Sig',@(x) isnumeric(x) && isvector(x) && (length(x) > 10));
addOptional(p,'m',2,Chkx);
addParameter(p,'tau',1,Chk);
addParameter(p,'Typex','none',@(x) ischar(x) && any(validatestring(lower(x),Chk3)));
    ...
    switch lower(Typex)        
        ...
        case 'edge'
            if isempty(tpx)
                tpx = 1;
            elseif tpx <=0 
                error('r sensitivity parameter (tpx) must be > 0')
            end
            if k > 1
                for n = 1:size(Px,1)
