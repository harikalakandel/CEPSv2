        ...
        case 'fir' 
            for idx = 1:J
                switch cg_moment
                    ...
                    case 'median'
                        scaledData(idx) = median(data((idx-1)*tau+1:idx*tau));
                    case 'minPoint'
                        scaledData(idx) = (max(data((idx-1)*tau+1:idx*tau)) + min(data((idx-1)*tau+1:idx*tau)))/2;
                    case 'kMeans'
                        currData = data((idx-1)*tau+1:idx*tau);
                         x_m = mean(currData);
                         z1 = x_m+0.0005*x_m;
                         z2 = x_m-0.0005*x_m;
                         loopCount = 1;
                         while 1==1
                            dist = [abs(currData - z1) abs(currData - z2)];
                            U = dist(:,1)<dist(:,2);
                            newZ1=mean(currData(U==1,1));
                            newZ2=mean(currData(U==0,1));
                            if newZ1 == z2
                                break;
                            end
                            z1=newZ1;
